-- อาซ้อฟ่างKGHUB Roll/Buy v7.0.2 Production + Clone + Drop Guide + Color Tags + Server Resume
...
if type(old)=="table" and old.alive~=false then
 ENV.__KGHUB_RERUN_RESUME={savedAt=os.time(),placeId=game.PlaceId,autoRoll=old.autoRoll==true,autoBuy=old.autoBuy==true,traderAuto=old.traderAuto==true,autoClone=(type(old.clone)=="table" and old.clone.enabled==true) or (type(old.autoLevel)=="table" and old.autoLevel.clonePaused==true and old.autoLevel.cloneWasOn==true),autoLevel=type(old.autoLevel)=="table" and old.autoLevel.enabled==true,eventCurrent=tostring(WS:GetAttribute("MutationEvent") or ""),eventLastHandled=type(old.eventPotion)=="table" and old.eventPotion.lastHandled or nil,eventLastResult=type(old.eventPotion)=="table" and old.eventPotion.lastResult or nil}
...
local TraderRemotes=RS:WaitForChild("Remotes"):FindFirstChild("Trader")
local TraderGetStock=TraderRemotes and TraderRemotes:FindFirstChild("GetStock")
local TraderBuy=TraderRemotes and TraderRemotes:FindFirstChild("Buy")
RT.traderAvailable=TraderGetStock~=nil and TraderBuy~=nil
...
do
 ...
 RT.writeAutoState=function(reason,armedOverride)
  ...
  local data={version=2,armed=armed==true,savedAt=os.time(),reason=tostring(reason or ""),placeId=game.PlaceId,jobId=tostring(game.JobId or ""),autoRoll=RT.autoRoll==true,autoBuy=RT.autoBuy==true,traderAuto=RT.traderAuto==true,autoClone=(RT.clone and RT.clone.enabled==true) or (RT.autoLevel and RT.autoLevel.clonePaused==true and RT.autoLevel.cloneWasOn==true),autoLevel=RT.autoLevel and RT.autoLevel.enabled==true}
...
local function cleanup(reason)
 ...
 RT.alive=false; RT.autoRoll=false; RT.autoBuy=false; RT.traderAuto=false; RT.traderBusy=false; RT.rollBuyHold=false; RT.rollBuyBatch=nil; RT.eventPotion.enabled=false; if RT.clone then RT.clone.enabled=false; RT.clone.busy=false end; if RT.autoLevel then RT.autoLevel.enabled=false; RT.autoLevel.busy=false end
...
RT.setupTeleportPersistence=function()
 ...
 local localName="KGHUB_latest.lua"
 local quotedRemote=string.format("%q",remoteURL)
 local quotedLocal=string.format("%q",localName)
 local loader="local E=(getgenv and getgenv()) or _G\nif E.__KGHUB_TP_BOOTED then return end\nE.__KGHUB_TP_BOOTED=true\nif game.PlaceId~=107653945083776 then E.__KGHUB_TP_BOOTED=nil return end\nif not game:IsLoaded() then game.Loaded:Wait() end\ntask.wait(1.5)\npcall(function() if type(writefile)==\"function\" then writefile(\"KGHUB_TP_BOOT_V1.txt\",tostring(os.time())..\"|\"..tostring(game.JobId or \"\")) end end)\nlocal src=nil\nlocal okLocal,vLocal=pcall(readfile,"..quotedLocal..")\nif okLocal and type(vLocal)==\"string\" and #vLocal>1000 then src=vLocal end\nif not src and "..quotedRemote.."~=\"\" then local okRemote,vRemote=pcall(function() return game:HttpGet("..quotedRemote..") end); if okRemote and type(vRemote)==\"string\" and #vRemote>1000 and string.find(vRemote,\"ui-v7.0.2-production\",1,true) then src=vRemote end end\nif not src then E.__KGHUB_TP_BOOTED=nil; warn(\"[KGHUB-TP] no local/remote source\"); return end\nlocal fn,err=loadstring(src)\nif not fn then E.__KGHUB_TP_BOOTED=nil; warn(\"[KGHUB-TP] compile failed\",err); return end\nlocal ok2,err2=pcall(fn)\nif not ok2 then E.__KGHUB_TP_BOOTED=nil; warn(\"[KGHUB-TP] run failed\",err2) end"
 ...
 if type(q)=="function" then
  ...
  log(ok and ("TP_QUEUE_READY | source=local_first | local="..localName.." | remote="..tostring(remoteURL~="")) or ("TP_QUEUE_ERROR | "..tostring(err)))
  RT.teleportQueueReady=ok; RT.teleportSource="local_first"; RT.teleportRemoteURL=remoteURL
 ...
 keep(LP.OnTeleport:Connect(function(state)
  ...
  local snap={savedAt=os.time(),placeId=game.PlaceId,autoRoll=RT.autoRoll==true,autoBuy=RT.autoBuy==true,traderAuto=RT.traderAuto==true,autoClone=(RT.clone and RT.clone.enabled==true) or (RT.autoLevel and RT.autoLevel.clonePaused==true and RT.autoLevel.cloneWasOn==true),autoLevel=RT.autoLevel and RT.autoLevel.enabled==true,eventCurrent=tostring(WS:GetAttribute("MutationEvent") or ""),eventLastHandled=RT.eventPotion.lastHandled,eventLastResult=RT.eventPotion.lastResult}
  local ok,json=pcall(function() return HttpService:JSONEncode(snap) end)
  if ok and type(writefile)=="function" then pcall(writefile,"KGHUB_v702_resume.json",json) end
  pcall(saveConfig); if RT.saveCloneConfig then pcall(RT.saveCloneConfig) end; if RT.saveLevelConfig then pcall(RT.saveLevelConfig) end
  log("TP_RESUME_SAVED | roll="..tostring(snap.autoRoll).." | buy="..tostring(snap.autoBuy).." | trader="..tostring(snap.traderAuto).." | clone="..tostring(snap.autoClone).." | level="..tostring(snap.autoLevel).." | event="..snap.eventCurrent)
 end))
end
...
local refreshTabs
local tabButtons={}
local tabOrder={"ROLL","RULES","TRADER","INFO","EVENT","CLONE","LEVEL","DROP","CONFIG"}
local tabLabels={ROLL="⚡  สุ่ม / ซื้อ",RULES="◆  กฎการซื้อ",TRADER="◈  ร้านค้า",INFO="◉  ข้อมูลตัวละคร",EVENT="✦  อีเวนต์ / ยา",CLONE="♊  Auto Clone",LEVEL="AUTO LEVEL",DROP="▣  DROP GUIDE",CONFIG="⚙  ตั้งค่า"}
...
RT.setupAutoClone=function()
 ...
 local editorPanel=Instance.new("Frame"); editorPanel.Position=UDim2.fromOffset(574,194); editorPanel.Size=UDim2.fromOffset(376,250); editorPanel.BackgroundColor3=Color3.fromRGB(16,21,32); editorPanel.BorderSizePixel=0; editorPanel.Parent=frame; stylePanel(editorPanel)
 local rulesPanel=Instance.new("Frame"); rulesPanel.Position=UDim2.fromOffset(574,454); rulesPanel.Size=UDim2.fromOffset(376,136); rulesPanel.BackgroundColor3=Color3.fromRGB(16,21,32); rulesPanel.BorderSizePixel=0; rulesPanel.Parent=frame; stylePanel(rulesPanel)
 ...
 U.maxLabel=tinyLabel("ต้องการเพิ่มอีก",12,96); U.delayLabel=tinyLabel("หน่วง (เสริม)",116,96); U.modeLabel=tinyLabel("นับแบบ (เสริม)",220,144)
 local maxBox=Instance.new("TextBox"); maxBox.Position=UDim2.fromOffset(12,120); maxBox.Size=UDim2.fromOffset(96,34); maxBox.BackgroundColor3=Color3.fromRGB(27,34,48); maxBox.BorderSizePixel=0; maxBox.Text="1"; maxBox.PlaceholderText="จำนวน"; maxBox.TextColor3=Color3.new(1,1,1); maxBox.FontFace=Font.new("rbxasset://fonts/families/BuilderSans.json"); maxBox.TextSize=14; maxBox.Parent=editorPanel; Instance.new("UICorner",maxBox).CornerRadius=UDim.new(0,8); styleTextBox(maxBox)
 ...
 local replaceBtn=mkButton(editorPanel,"รักษาจำนวน: ปิด",12,162,144,36); replaceBtn.TextSize=12
 local saveBtn=mkButton(editorPanel,"▶ AUTO เพิ่ม",164,162,116,36); saveBtn.TextSize=13; saveBtn.BackgroundColor3=Color3.fromRGB(44,143,101)
 local nowBtn=mkButton(editorPanel,"+1 ตอนนี้",288,162,76,36); nowBtn.TextSize=12; nowBtn.BackgroundColor3=Color3.fromRGB(54,89,170)
 ...
 local ruleScroll=Instance.new("ScrollingFrame"); ruleScroll.Position=UDim2.fromOffset(8,42); ruleScroll.Size=UDim2.fromOffset(360,86); ruleScroll.BackgroundTransparency=1; ruleScroll.BorderSizePixel=0; ruleScroll.ScrollBarThickness=5; ruleScroll.Parent=rulesPanel; styleScroll(ruleScroll)
 ...
 local draftReplace=false; local draftMode="exact"
 U.advancedOpen=false; U.advancedBtn=mkButton(editorPanel,"⚙ ตัวเลือกเสริม",12,218,352,24); U.advancedBtn.TextSize=11
 U.applyCloneAdvanced=function() local v=U.advancedOpen==true; delayBox.Visible=v; modeBtn.Visible=v; replaceBtn.Visible=v; U.delayLabel.Visible=v; U.modeLabel.Visible=v; U.advancedBtn.Text=v and "▲ ซ่อนตัวเลือกเสริม" or "⚙ ตัวเลือกเสริม" end
 keep(U.advancedBtn.Activated:Connect(function() U.advancedOpen=not U.advancedOpen; U.applyCloneAdvanced() end)); U.applyCloneAdvanced()
 ...
 local function refreshEditor()
  ...
  modeBtn.Text=draftMode=="exact" and "ตัวแบบเดียวกัน" or "ชื่อเดียวกันทั้งหมด"
  modeBtn.BackgroundColor3=draftMode=="exact" and Color3.fromRGB(50,70,112) or Color3.fromRGB(64,73,92)
  replaceBtn.Text=draftReplace and "↻ รักษาจำนวน: เปิด" or "↻ รักษาจำนวน: ปิด"
  replaceBtn.BackgroundColor3=draftReplace and Color3.fromRGB(43,112,86) or Color3.fromRGB(61,68,82)
  saveBtn.Text=C.selectedRuleId and "✓ อัปเดต + ทำต่อ" or "▶ AUTO เพิ่ม"
  local du=selectedUnit(); if du then local cur=countRule({matchMode=draftMode,name=tostring(du.Name),sourceSig=sig(du)}); local add=math.clamp(math.floor(tonumber(maxBox.Text) or 1),1,999); note.Text="ตอนนี้มี "..cur.." • จะเพิ่ม "..add.." • เป้ารวม "..math.min(999,cur+add).." • Clone ตามต้นฉบับ"; note.TextColor3=Color3.fromRGB(111,211,180) else note.Text="เลือกยูนิตด้านซ้ายก่อน"; note.TextColor3=Color3.fromRGB(123,143,176) end
 end
 ...
 local function refreshSources()
  ...
  for _,u in ipairs(inv()) do
   ...
   if ok and (q=="" or string.find(hay,q,1,true)) then
    ...
    b.Activated:Connect(function() C.selectedSourceUUID=uid(u); C.selectedRuleId=nil; draftReplace=false; draftMode="exact"; maxBox.Text="1"; delayBox.Text="2"; refreshSources(); refreshEditor() end)
   end
  end
 end
 local function refreshRules()
  ...
  for _,r in ipairs(C.rules) do
   ...
   edit.Activated:Connect(function() C.selectedRuleId=r.id; C.selectedSourceUUID=r.sourceUUID; draftReplace=r.autoReplace==true; draftMode=r.matchMode or "exact"; maxBox.Text=tostring(math.max(1,(tonumber(r.maxCount) or 1)-countRule(r))); delayBox.Text=tostring(r.delay); refreshSources(); refreshEditor(); refreshRules() end)
 ...
 RT.refreshCloneUI=function()
  ...
  else msg=#C.rules>0 and ("พร้อม • มีกฎ "..#C.rules.." กฎ • กดเริ่มกฎเดิมได้") or "1 เลือกยูนิต → 2 ใส่จำนวน → 3 กด AUTO เพิ่ม" end
  C.status=msg
  master.Text=C.enabled and "■  หยุด AUTO CLONE" or (#C.rules>0 and "▶  เริ่มกฎเดิม" or "▶  AUTO CLONE")
 ...
 keep(maxBox:GetPropertyChangedSignal("Text"):Connect(function() refreshEditor() end))
 ...
 keep(saveBtn.Activated:Connect(function()
  ...
  local m=metaFor(u.Name); local add=math.clamp(math.floor(tonumber(maxBox.Text) or 1),1,999); local current=countRule({matchMode=draftMode,name=tostring(u.Name),sourceSig=sig(u)}); local max=math.clamp(current+add,1,999); local delay=math.clamp(tonumber(delayBox.Text) or 2,0,3600)
  ...
  C.selectedRuleId=r.id; C.enabled=true; C.busy=false; C.nextActionAt=os.clock(); C.claimedUUID=nil; RT.saveCloneConfig(); log("CLONE_RULE_SAVE_START | id="..r.id.." | name="..r.name.." | add="..add.." | current="..current.." | max="..max.." | delay="..delay.." | replace="..tostring(r.autoReplace)); RT.refreshCloneUI(); refreshRules()
 end))
...
RT.setupAutoClone()
RT.setupAutoLevel=function()
 local AnimeState=require(Modules:WaitForChild("Shared"):WaitForChild("AnimeState")); AnimeState.Start(); local dataClient=require(RS:WaitForChild("Data"):WaitForChild("DataService")).client; local CloneInfo=require(Modules:WaitForChild("Shared"):WaitForChild("CloneStuff"):WaitForChild("CloneInfo")); local Request=RS:WaitForChild("Remotes"):WaitForChild("CloneRemotes"):WaitForChild("Request"); local Fight=RS:WaitForChild("Remotes"):WaitForChild("Fight"):WaitForChild("Start"); local VIM=game:GetService("VirtualInputManager")
 RT.autoLevel=RT.autoLevel or {enabled=false,busy=false,status="หยุด",sourceUUID=nil,name=nil,mutation="",trait="",baseLevel=1,targetLevel=2,targetCount=1,clonePaused=false,cloneWasOn=false}; local L=RT.autoLevel; L.file="KGHUB_AUTO_LEVEL.json"; RT.levelUI={group={}}; local U=RT.levelUI
 local function uid(x) return tostring(x and (x.UUID or x.CharacterId) or "") end; local function inv() return AnimeState.GetInventory() or {} end; local function meta(name) for _,m in ipairs(CharacterInfo.Characters or {}) do if tostring(m.Name)==tostring(name) then return m end end end; local function sameBase(u) return u and tostring(u.Name)==tostring(L.name or "") and tostring(u.Mutation or "")==tostring(L.mutation or "") and tostring(u.Trait or "")==tostring(L.trait or "") end
 local function maxLv(u) local m=meta(u and u.Name or L.name); return tostring(m and m.Rarity or "")=="Supreme" and 8 or 7 end; local function activeClone() for _,x in ipairs(AnimeState.GetCloning() or {}) do if uid(x)~="" then return x end end end; local function findTool(id) for _,box in ipairs({LP.Character,LP:FindFirstChild("Backpack")}) do if box then for _,t in ipairs(box:GetChildren()) do if t:IsA("Tool") and tostring(t:GetAttribute("UUID"))==tostring(id) then return t end end end end end
 local function highestSource() local best=nil; for _,u in ipairs(inv()) do local lv=tonumber(u.Level) or 1; if sameBase(u) and lv<=tonumber(L.targetLevel or 1) then local bl=best and (tonumber(best.Level) or 1) or -1; if (not best) or lv>bl or (lv==bl and uid(u)==tostring(L.sourceUUID or "")) then best=u end end end; return best end; local function countFinal() local n=0; for _,u in ipairs(inv()) do if sameBase(u) and tonumber(u.Level)==tonumber(L.targetLevel) then n+=1 end end; return n end
 local function spareMaterial(src,excludeId) local sid=uid(src); local keepId=tostring(L.sourceUUID or ""); local wantLv=tonumber(src and src.Level) or 1; for _,u in ipairs(inv()) do local id=uid(u); if id~="" and id~=sid and id~=keepId and id~=tostring(excludeId or "") and sameBase(u) and tonumber(u.Level)==wantLv and findTool(id) then return u end end end
 local function eligible(u) local m=u and meta(u.Name); if not m then return false,"ไม่พบข้อมูลตัวละคร" end; if tostring(m.Rarity)=="Limited" then return false,"Limited Clone ไม่ได้" end; if tostring(u.Mutation or "")=="Astronaut" then return false,"Astronaut Clone ไม่ได้" end; if m.CanClone==false then return false,"เกมปิด CanClone" end; return uid(u)~="","พร้อม" end
 RT.saveLevelConfig=function() if type(writefile)~="function" then return false end; local d={version="1.4-half-merge",sourceUUID=L.sourceUUID,name=L.name,mutation=L.mutation,trait=L.trait,baseLevel=L.baseLevel,targetLevel=L.targetLevel,targetCount=L.targetCount}; local ok,j=pcall(function() return HttpService:JSONEncode(d) end); return ok and pcall(writefile,L.file,j) end
 RT.loadLevelConfig=function() if type(readfile)~="function" then return false end; local ok,t=pcall(readfile,L.file); if not ok or type(t)~="string" or t=="" then return false end; local ok2,d=pcall(function() return HttpService:JSONDecode(t) end); if not ok2 or type(d)~="table" then return false end; L.sourceUUID=tostring(d.sourceUUID or ""); L.name=tostring(d.name or ""); L.mutation=tostring(d.mutation or ""); L.trait=tostring(d.trait or ""); L.baseLevel=math.max(1,math.floor(tonumber(d.baseLevel) or 1)); L.targetLevel=math.max(L.baseLevel,math.floor(tonumber(d.targetLevel) or L.baseLevel)); L.targetCount=math.clamp(math.floor(tonumber(d.targetCount) or 1),1,99); return true end; RT.loadLevelConfig()
 local function setStatus(s) L.status=tostring(s or ""); if RT.refreshLevelUI then RT.refreshLevelUI() end end; local function pauseClone() if L.clonePaused then return end; L.cloneWasOn=RT.clone and RT.clone.enabled==true or false; if RT.clone then RT.clone.enabled=false; RT.clone.busy=false end; L.clonePaused=true end; local function restoreClone() if not L.clonePaused then return end; if RT.clone and L.cloneWasOn then RT.clone.enabled=true; RT.clone.busy=false; RT.clone.nextActionAt=os.clock()+2 end; L.clonePaused=false; L.cloneWasOn=false end
 local function waitFor(fn,seconds,step) local untilAt=os.clock()+(tonumber(seconds) or 5); repeat local ok,v=pcall(fn); if ok and v then return v end; task.wait(step or .1) until os.clock()>=untilAt or not RT.alive; return nil end
 local function cloneOne(src) local ok,why=eligible(src); if not ok then return nil,why end; if activeClone() then return nil,"เครื่อง Clone ยังไม่ว่าง" end; local m=meta(src.Name); local item=tostring(m and m.Rarity or "Common").." Essence"; local have=math.floor(tonumber(AnimeState.GetItemAmount(item)) or 0); local need=math.floor(tonumber(CloneInfo.GetRequiredFragments(src.Level,src.Mutation)) or 0); if have<need then return nil,item.." ไม่พอ "..have.."/"..need end; local before={}; for _,u in ipairs(inv()) do before[uid(u)]=true end; local sid=uid(src); local sent,err=pcall(function() Request:FireServer("Start",{UUID=sid,CharacterId=sid}) end); if not sent then return nil,"Start error: "..tostring(err) end; local row=waitFor(function() local a=activeClone(); if a and uid(a)==sid then return a end end,6,.15); if not row then return nil,"เครื่อง Clone ไม่รับคำสั่ง" end
 while RT.alive and L.enabled do
  local current=activeClone()
  if not current or uid(current)~=sid then return nil,"clone canceled or replaced" end
  row=current
  local left=tonumber(AnimeState.GetCloneLeft(row)) or 0
  if left<=0 then break end
  setStatus("กำลัง Clone "..tostring(src.Name).." Lv."..tostring(src.Level).." • เหลือ "..math.ceil(left).."s")
  task.wait(.5)
 end
 if not RT.alive then return nil,"script stopped" end
 if not L.enabled then return nil,"auto level stopped" end
 local current=activeClone(); if not current or uid(current)~=sid then return nil,"clone canceled before claim" end
 local claimOk,claimErr=pcall(function() Request:FireServer("Claim",{UUID=sid,CharacterId=sid}) end); if not claimOk then return nil,"Claim error: "..tostring(claimErr) end
 local fresh=waitFor(function() for _,u in ipairs(inv()) do if not before[uid(u)] and tostring(u.Name)==tostring(src.Name) and tonumber(u.Level)==tonumber(src.Level) and tostring(u.Mutation or "")==tostring(src.Mutation or "") and tostring(u.Trait or "")==tostring(src.Trait or "") then return u end end end,8,.15); if not fresh then return nil,"Claim แล้วแต่หา clone ใหม่ไม่พบ" end; waitFor(function() return findTool(uid(fresh)) end,5,.1); return fresh,nil
 end
 local function ownerPlot() for _,p in ipairs(WS:WaitForChild("Plots"):GetChildren()) do if p:GetAttribute("Owner")==LP.Name then return p end end end
 local function emptySlot() local plot=ownerPlot(); if not plot then return nil,nil,nil end; local used={}; for _,e in ipairs(dataClient:get("Equipped") or {}) do local s=tonumber(e.Slot); if s then used[s]=true end end; local worldUsed={}; local chars=plot:FindFirstChild("Characters"); if chars then for _,m in ipairs(chars:GetChildren()) do local s=tonumber(m:GetAttribute("Slot")); if s then worldUsed[s]=true end end end; local unlocked=math.max(1,tonumber(dataClient:get({"Upgrades","Slots"})) or 1); for i=1,unlocked do local slot=plot:FindFirstChild("Placement") and plot.Placement:FindFirstChild("slot"..i); local pr=slot and slot:FindFirstChild("PlacementPrompt",true); if slot and pr and not worldUsed[i] and ((not used[i]) or tostring(pr.ActionText)=="Place") then return plot,slot,i end end end
 local function mergePair(a,b,src) local ta=findTool(uid(a)); local tb=findTool(uid(b)); if not ta or not tb then return nil,"Tool ของ Clone ไม่ครบ" end; local plot,slot,slotNum=emptySlot(); if not slot then return nil,"ต้องมีช่องว่างในสนามอย่างน้อย 1 ช่อง" end; local pr=slot:FindFirstChild("PlacementPrompt",true); local att=pr and pr.Parent; local ch=LP.Character; local hum=ch and ch:FindFirstChildOfClass("Humanoid"); local root=ch and ch:FindFirstChild("HumanoidRootPart"); if not pr or not hum or not root then return nil,"ไม่พบ PlacementPrompt/Character" end; local oldCF=root.CFrame; local wasAuto=LP:GetAttribute("FightAutoPlay")==true; local wasFight=LP:GetAttribute("FightRunning")==true
 local function restoreWorld() pcall(function() root.CFrame=oldCF end); if wasAuto and LP:GetAttribute("FightAutoPlay")~=true then Fight:FireServer("AutoPlay"); task.wait(.2) end; if wasFight and LP:GetAttribute("FightRunning")~=true then Fight:FireServer("Start"); task.wait(.2) end end; local function occ() for _,m in ipairs(plot:WaitForChild("Characters"):GetChildren()) do if tonumber(m:GetAttribute("Slot"))==slotNum then return m end end end; local function press() local pos=att:IsA("Attachment") and att.WorldPosition or slot:GetPivot().Position; root.CFrame=CFrame.new(pos+Vector3.new(0,2.5,0)); task.wait(.35); local k=pr.KeyboardKeyCode; VIM:SendKeyEvent(true,k,false,game); task.wait((pr.HoldDuration or 0)+.25); VIM:SendKeyEvent(false,k,false,game); task.wait(.85) end
 if wasAuto then Fight:FireServer("AutoPlay"); if not waitFor(function() return LP:GetAttribute("FightAutoPlay")~=true end,3,.1) then restoreWorld(); return nil,"ปิด AutoPlay ไม่สำเร็จ" end end; if LP:GetAttribute("FightRunning")==true then Fight:FireServer("Stop"); if not waitFor(function() return LP:GetAttribute("FightRunning")~=true end,4,.1) then restoreWorld(); return nil,"หยุด Fight ไม่สำเร็จ" end end; task.wait(.35)
 local ok,why=true,nil; hum:EquipTool(ta); task.wait(.35); if not waitFor(function() return pr.ActionText=="Place" end,2,.1) then ok=false; why="ช่องผสมไม่พร้อม ("..tostring(pr.ActionText)..")" else press() end; if ok then hum:EquipTool(tb); task.wait(.35); if not waitFor(function() return pr.ActionText=="Level Up!" end,2,.1) then ok=false; why="เกมไม่ยอม Merge ("..tostring(pr.ActionText)..")" else press() end end
 local m=occ(); local wantLv=(tonumber(src.Level) or 1)+1; if ok then ok=m and tostring(m:GetAttribute("CharacterName") or m.Name)==tostring(src.Name) and tonumber(m:GetAttribute("Level"))==wantLv and tostring(m:GetAttribute("Mutation") or "")==tostring(src.Mutation or ""); if not ok then why="ผล Merge ไม่ตรงที่คาด" end end; local resultId=m and tostring(m:GetAttribute("UUID") or "") or ""
 if ok then waitFor(function() return pr.ActionText=="Pick Up" end,2,.1); if pr.ActionText=="Pick Up" then press() else ok=false; why="หยิบผลลัพธ์กลับไม่ได้" end end; local result=nil; if ok then result=waitFor(function() for _,u in ipairs(inv()) do if (uid(u)==resultId or uid(u)==uid(a)) and tostring(u.Name)==tostring(src.Name) and tonumber(u.Level)==wantLv and tostring(u.Mutation or "")==tostring(src.Mutation or "") then return u end end end,5,.15); if not result then ok=false; why="Merge สำเร็จแต่หา Lv ใหม่ใน Inventory ไม่พบ" end end; restoreWorld(); return ok and result or nil,why
 end
 local function resumeHalfMerge()
  local plot=ownerPlot(); if not plot then return false,nil end
  local chars=plot:FindFirstChild("Characters"); local placement=plot:FindFirstChild("Placement"); if not chars or not placement then return false,nil end
  local goal=tonumber(L.targetLevel) or 1
  for _,m in ipairs(chars:GetChildren()) do
   local lv=tonumber(m:GetAttribute("Level")); local nm=tostring(m:GetAttribute("CharacterName") or m.Name); local mu=tostring(m:GetAttribute("Mutation") or ""); local tr=tostring(m:GetAttribute("Trait") or "")
   if nm==tostring(L.name or "") and mu==tostring(L.mutation or "") and tr==tostring(L.trait or "") and lv and lv<goal then
    local sn=tonumber(m:GetAttribute("Slot")); local slot=sn and placement:FindFirstChild("slot"..sn); local pr=slot and slot:FindFirstChild("PlacementPrompt",true)
    if pr and tostring(pr.ActionText)=="Level Up!" then
     setStatus("RESUME MERGE ? ?? Lv."..tostring(lv).." ??????? slot "..tostring(sn))
     local material=nil
     for _,u in ipairs(inv()) do local id=uid(u); if id~="" and id~=tostring(L.sourceUUID or "") and sameBase(u) and tonumber(u.Level)==lv and findTool(id) then material=u; break end end
     if not material then
      local seed=highestSource()
      if not seed or tonumber(seed.Level)~=lv then setStatus("RESUME MERGE ? ????? seed Lv."..tostring(lv)); return true,"no_seed" end
      setStatus("RESUME MERGE ? ??????????? 1 ??? ????? Clone Lv."..tostring(lv))
      local made,er=cloneOne(seed); if not made then setStatus("RESUME MERGE ? "..tostring(er)); return true,"clone_failed" end; material=made
     end
     local tool=findTool(uid(material)); if not tool then tool=waitFor(function() return findTool(uid(material)) end,5,.1) end
     local ch=LP.Character; local hum=ch and ch:FindFirstChildOfClass("Humanoid"); local root=ch and ch:FindFirstChild("HumanoidRootPart"); if not tool or not hum or not root then setStatus("RESUME MERGE ? ????? Tool/Character"); return true,"no_tool" end
     local oldCF=root.CFrame; local wasAuto=LP:GetAttribute("FightAutoPlay")==true; local wasFight=LP:GetAttribute("FightRunning")==true
     local function restore() pcall(function() root.CFrame=oldCF end); if wasAuto and LP:GetAttribute("FightAutoPlay")~=true then Fight:FireServer("AutoPlay"); task.wait(.2) end; if wasFight and LP:GetAttribute("FightRunning")~=true then Fight:FireServer("Start"); task.wait(.2) end end
     if wasAuto then Fight:FireServer("AutoPlay"); waitFor(function() return LP:GetAttribute("FightAutoPlay")~=true end,3,.1) end
     if LP:GetAttribute("FightRunning")==true then Fight:FireServer("Stop"); waitFor(function() return LP:GetAttribute("FightRunning")~=true end,4,.1) end
     hum:EquipTool(tool); task.wait(.3)
     local att=pr.Parent; local pos=att:IsA("Attachment") and att.WorldPosition or slot:GetPivot().Position; root.CFrame=CFrame.new(pos+Vector3.new(0,2.5,0)); task.wait(.25)
     if not waitFor(function() return tostring(pr.ActionText)=="Level Up!" end,2,.1) then restore(); setStatus("RESUME MERGE ? prompt ????????: "..tostring(pr.ActionText)); return true,"prompt_not_ready" end
     local ok,er=pcall(function() fireproximityprompt(pr) end); if not ok then restore(); setStatus("RESUME MERGE ? fire error: "..tostring(er)); return true,"fire_error" end
     local resultModel=waitFor(function() for _,x in ipairs(chars:GetChildren()) do if tonumber(x:GetAttribute("Slot"))==sn and tostring(x:GetAttribute("CharacterName") or x.Name)==nm and tonumber(x:GetAttribute("Level"))==lv+1 then return x end end end,5,.1)
     if not resultModel then restore(); setStatus("RESUME MERGE ? Merge ?????????"); return true,"merge_not_confirmed" end
     local rid=tostring(resultModel:GetAttribute("UUID") or "")
     waitFor(function() return tostring(pr.ActionText)=="Pick Up" end,2,.1)
     if tostring(pr.ActionText)=="Pick Up" then pcall(function() fireproximityprompt(pr) end) else restore(); setStatus("RESUME MERGE ? Pick Up ????????"); return true,"pickup_not_ready" end
     local result=waitFor(function() for _,u in ipairs(inv()) do if (uid(u)==rid or (sameBase(u) and tonumber(u.Level)==lv+1)) then return u end end end,6,.15)
     restore()
     if not result then setStatus("RESUME MERGE ? ??? Lv ??????????? Inventory ?????"); return true,"result_missing" end
     setStatus("RESUME MERGE ? ?????? "..tostring(result.Name).." Lv."..tostring(result.Level))
     log("AUTO_LEVEL_HALF_MERGE_RECOVERED | name="..tostring(result.Name).." | level="..tostring(result.Level).." | slot="..tostring(sn))
 ...
 local function resumeExistingClone()
  local row=activeClone(); if not row then return false,nil end
  if not sameBase(row) then setStatus("RESUME ? ????????? Clone ??????????????"); return true,"foreign" end
  local sid=uid(row); if sid=="" then return true,"bad_uuid" end
  pauseClone()
  while RT.alive and L.enabled do
   local cur=activeClone()
   if not cur then return true,"gone" end
   if uid(cur)~=sid then return true,"replaced" end
   local left=tonumber(AnimeState.GetCloneLeft(cur)) or 0
   if left<=0 then
    setStatus("RESUME ? Clone ????????????? ????? Claim")
    local ok,err=pcall(function() Request:FireServer("Claim",{UUID=sid,CharacterId=sid}) end)
    if not ok then return true,"claim error: "..tostring(err) end
    waitFor(function() local a=activeClone(); return (not a) or uid(a)~=sid end,8,.15)
    setStatus("RESUME ? ??? Clone ???????? ? ?????????????")
    return true,"claimed"
   end
   setStatus("RESUME ? ??????? Clone "..tostring(cur.Name or L.name).." Lv."..tostring(cur.Level or "?").." ? ????? "..math.ceil(left).."s")
   task.wait(.5)
  end
  return true,"stopped"
 end
 local function stopJob(reason) L.enabled=false; L.busy=false; L.status=reason or "หยุด"; restoreClone(); RT.saveLevelConfig(); if RT.refreshLevelUI then RT.refreshLevelUI() end end
 local function runCycle()
  local goalLevel=math.floor(tonumber(L.targetLevel) or 1)
  local goalCount=math.max(1,math.floor(tonumber(L.targetCount) or 1))
  local done=countFinal()
  if done>=goalCount then stopJob("\u{E40}\u{E2A}\u{E23}\u{E47}\u{E08}\u{E41}\u{E25}\u{E49}\u{E27} \u{2022} Lv."..tostring(goalLevel).." x"..tostring(done).."/"..tostring(goalCount)); return end
  local src=highestSource()
  if not src then setStatus("\u{E2B}\u{E32} source \u{E17}\u{E35}\u{E48}\u{E15}\u{E23}\u{E07}\u{E01}\u{E31}\u{E1A}\u{E15}\u{E31}\u{E27}\u{E17}\u{E35}\u{E48}\u{E40}\u{E25}\u{E37}\u{E2D}\u{E01}\u{E44}\u{E21}\u{E48}\u{E1E}\u{E1A} \u{2022} \u{E23}\u{E2D} Inventory \u{E2D}\u{E31}\u{E1B}\u{E40}\u{E14}\u{E15}"); task.wait(1.25); return end
  local ml=maxLv(src)
  if goalLevel>ml then L.targetLevel=ml; goalLevel=ml; RT.saveLevelConfig() end
  local srcLv=tonumber(src.Level) or 1
  if srcLv<goalLevel then
   local a=spareMaterial(src,nil); local e1=nil
   if a then setStatus("BUILD LEVEL ? ??????????????? 1/2 ? Lv."..tostring(srcLv).." ? Clone ???????????") else setStatus("BUILD LEVEL ? ????????????? 1/2 ? Lv."..tostring(srcLv).." -> Lv."..tostring(srcLv+1).." ? ???? Lv."..tostring(goalLevel)); a,e1=cloneOne(src) end
   if not a then setStatus("BUILD LEVEL ? "..tostring(e1)); task.wait(1.25); return end
   if not L.enabled then return end
   local b=spareMaterial(src,uid(a)); local e2=nil
   if b then setStatus("BUILD LEVEL ? ??????????????? 2/2 ? Lv."..tostring(srcLv).." ? ????? Merge") else setStatus("BUILD LEVEL ? ????????????? 2/2 ? Lv."..tostring(srcLv).." -> Lv."..tostring(srcLv+1).." ? ???? Lv."..tostring(goalLevel)); b,e2=cloneOne(src) end
   if not b then setStatus("BUILD LEVEL ? "..tostring(e2)); task.wait(1.25); return end
   if not L.enabled then return end
   setStatus("BUILD LEVEL \u{2022} Merge Lv."..tostring(srcLv).." + Lv."..tostring(srcLv).." -> Lv."..tostring(srcLv+1))
   local r,em=mergePair(a,b,src); if not r then setStatus("BUILD LEVEL \u{2022} "..tostring(em)); task.wait(1.4); return end
   setStatus("BUILD LEVEL \u{2022} \u{E2A}\u{E33}\u{E40}\u{E23}\u{E47}\u{E08} "..tostring(r.Name).." Lv."..tostring(r.Level).." \u{2022} \u{E01}\u{E33}\u{E25}\u{E31}\u{E07}\u{E44}\u{E15}\u{E48}\u{E15}\u{E48}\u{E2D}\u{E16}\u{E36}\u{E07} Lv."..tostring(goalLevel))
   log("AUTO_LEVEL_BUILD | name="..tostring(r.Name).." | madeLv="..tostring(r.Level).." | goalLv="..tostring(goalLevel).." | goalCount="..tostring(goalCount))
  else
   done=countFinal()
   if done>=goalCount then stopJob("\u{E40}\u{E2A}\u{E23}\u{E47}\u{E08}\u{E41}\u{E25}\u{E49}\u{E27} \u{2022} Lv."..tostring(goalLevel).." x"..tostring(done).."/"..tostring(goalCount)); return end
   setStatus("MASS CLONE \u{2022} Lv."..tostring(goalLevel).." \u{2022} "..tostring(done).."/"..tostring(goalCount).." \u{2022} \u{E01}\u{E33}\u{E25}\u{E31}\u{E07}\u{E42}\u{E04}\u{E25}\u{E19}\u{E40}\u{E1E}\u{E34}\u{E48}\u{E21}")
   local u,er=cloneOne(src); if not u then setStatus("MASS CLONE \u{2022} "..tostring(er)); task.wait(1.25); return end
   local now=countFinal()
   setStatus("MASS CLONE \u{2022} \u{E44}\u{E14}\u{E49} Lv."..tostring(goalLevel).." \u{E41}\u{E25}\u{E49}\u{E27} "..tostring(now).."/"..tostring(goalCount))
   log("AUTO_LEVEL_MASS | name="..tostring(u.Name).." | level="..tostring(goalLevel).." | count="..tostring(now).."/"..tostring(goalCount))
   if now>=goalCount then stopJob("\u{E40}\u{E2A}\u{E23}\u{E47}\u{E08}\u{E41}\u{E25}\u{E49}\u{E27} \u{2022} Lv."..tostring(goalLevel).." x"..tostring(now).."/"..tostring(goalCount)) end
  end
 end
 task.spawn(function() while RT.alive do task.wait(.45); if L.enabled and not L.busy then L.busy=true; local ok,err=pcall(function() if activeClone() then local handled,note=resumeExistingClone(); if handled and note=="foreign" then task.wait(.8) end else pauseClone(); local half,note=resumeHalfMerge(); if not half then runCycle() elseif note~="completed" then task.wait(.8) end end end); L.busy=false; if not ok then setStatus("Auto Level error: "..tostring(err)); task.wait(1) end; if not L.enabled then restoreClone() end end end end)
 local title=Instance.new("TextLabel"); title.BackgroundTransparency=1; title.Position=UDim2.fromOffset(210,82); title.Size=UDim2.fromOffset(500,34); title.FontFace=Font.new("rbxasset://fonts/families/BuilderSans.json",Enum.FontWeight.Bold,Enum.FontStyle.Normal); title.TextSize=25; title.TextXAlignment=Enum.TextXAlignment.Left; title.TextColor3=Color3.fromRGB(248,250,255); title.Text="⬆ Auto Level"; title.Parent=frame
 local sub=Instance.new("TextLabel"); sub.BackgroundTransparency=1; sub.Position=UDim2.fromOffset(212,113); sub.Size=UDim2.fromOffset(520,20); sub.FontFace=Font.new("rbxasset://fonts/families/BuilderSans.json"); sub.TextSize=12; sub.TextXAlignment=Enum.TextXAlignment.Left; sub.TextColor3=Color3.fromRGB(139,158,193); sub.Text="\u{E40}\u{E25}\u{E37}\u{E2D}\u{E01}\u{E15}\u{E31}\u{E27} -> \u{E40}\u{E25}\u{E37}\u{E2D}\u{E01} Lv \u{E40}\u{E1B}\u{E49}\u{E32} + \u{E08}\u{E33}\u{E19}\u{E27}\u{E19} -> Auto Level \u{E04}\u{E38}\u{E21} Auto Clone \u{E40}\u{E2D}\u{E07} -> \u{E44}\u{E15}\u{E48} Lv -> \u{E42}\u{E04}\u{E25}\u{E19}\u{E40}\u{E1E}\u{E34}\u{E48}\u{E21}\u{E08}\u{E19}\u{E04}\u{E23}\u{E1A}\u{E08}\u{E33}\u{E19}\u{E27}\u{E19}"; sub.Parent=frame
 local master=mkButton(frame,"▶ เริ่ม AUTO LEVEL",742,84,208,46); master.TextSize=15; master.BackgroundColor3=Color3.fromRGB(47,79,142); local status=Instance.new("TextLabel"); status.Position=UDim2.fromOffset(210,146); status.Size=UDim2.fromOffset(740,38); status.BackgroundColor3=Color3.fromRGB(18,24,36); status.BorderSizePixel=0; status.FontFace=Font.new("rbxasset://fonts/families/BuilderSans.json",Enum.FontWeight.Medium,Enum.FontStyle.Normal); status.TextSize=13; status.TextColor3=Color3.fromRGB(205,219,244); status.TextXAlignment=Enum.TextXAlignment.Left; status.Text="  หยุด"; status.Parent=frame; Instance.new("UICorner",status).CornerRadius=UDim.new(0,10)
 local left=Instance.new("Frame"); left.Position=UDim2.fromOffset(210,194); left.Size=UDim2.fromOffset(350,396); left.BackgroundColor3=Color3.fromRGB(16,21,32); left.BorderSizePixel=0; left.Parent=frame; stylePanel(left); local right=Instance.new("Frame"); right.Position=UDim2.fromOffset(574,194); right.Size=UDim2.fromOffset(376,396); right.BackgroundColor3=Color3.fromRGB(16,21,32); right.BorderSizePixel=0; right.Parent=frame; stylePanel(right); U.group={title,sub,master,status,left,right}; U.master=master; U.status=status
 local lh=Instance.new("TextLabel"); lh.BackgroundTransparency=1; lh.Position=UDim2.fromOffset(12,8); lh.Size=UDim2.fromOffset(326,28); lh.FontFace=Font.new("rbxasset://fonts/families/BuilderSans.json",Enum.FontWeight.Bold,Enum.FontStyle.Normal); lh.TextSize=18; lh.TextXAlignment=Enum.TextXAlignment.Left; lh.TextColor3=Color3.fromRGB(240,245,255); lh.Text="1  เลือกยูนิตต้นแบบ"; lh.Parent=left; local search=Instance.new("TextBox"); search.Position=UDim2.fromOffset(10,44); search.Size=UDim2.fromOffset(330,38); search.BackgroundColor3=Color3.fromRGB(25,32,47); search.BorderSizePixel=0; search.ClearTextOnFocus=false; search.PlaceholderText="ค้นหาชื่อ / Mutation / Trait"; search.Text=""; search.TextColor3=Color3.new(1,1,1); search.FontFace=Font.new("rbxasset://fonts/families/BuilderSans.json"); search.TextSize=14; search.Parent=left; Instance.new("UICorner",search).CornerRadius=UDim.new(0,8); styleTextBox(search)
 local scroll=Instance.new("ScrollingFrame"); scroll.Position=UDim2.fromOffset(8,90); scroll.Size=UDim2.fromOffset(334,296); scroll.BackgroundTransparency=1; scroll.BorderSizePixel=0; scroll.ScrollBarThickness=5; scroll.Parent=left; styleScroll(scroll); local lay=Instance.new("UIListLayout"); lay.Padding=UDim.new(0,6); lay.Parent=scroll
 local rh=lh:Clone(); rh.Position=UDim2.fromOffset(14,10); rh.Size=UDim2.fromOffset(348,28); rh.Text="2  ตั้งเป้าหมาย"; rh.Parent=right; local sel=Instance.new("TextLabel"); sel.Position=UDim2.fromOffset(12,48); sel.Size=UDim2.fromOffset(352,72); sel.BackgroundColor3=Color3.fromRGB(22,29,43); sel.BorderSizePixel=0; sel.TextWrapped=true; sel.TextXAlignment=Enum.TextXAlignment.Left; sel.TextYAlignment=Enum.TextYAlignment.Center; sel.FontFace=Font.new("rbxasset://fonts/families/BuilderSans.json",Enum.FontWeight.Medium,Enum.FontStyle.Normal); sel.TextSize=13; sel.TextColor3=Color3.fromRGB(220,230,246); sel.Text="  ยังไม่ได้เลือกตัว"; sel.Parent=right; Instance.new("UICorner",sel).CornerRadius=UDim.new(0,9)
 local lvMinus=mkButton(right,"−",14,146,48,40); local lvText=mkButton(right,"Lv.2",70,146,116,40); lvText.AutoButtonColor=false; local lvPlus=mkButton(right,"+",194,146,48,40); local cntMinus=mkButton(right,"−",14,218,48,40); local cntText=mkButton(right,"จำนวน 1",70,218,116,40); cntText.AutoButtonColor=false; local cntPlus=mkButton(right,"+",194,218,48,40)
 local lvLab=Instance.new("TextLabel"); lvLab.BackgroundTransparency=1; lvLab.Position=UDim2.fromOffset(14,126); lvLab.Size=UDim2.fromOffset(228,18); lvLab.Text="Level เป้าหมาย"; lvLab.TextXAlignment=Enum.TextXAlignment.Left; lvLab.TextColor3=Color3.fromRGB(137,157,192); lvLab.FontFace=Font.new("rbxasset://fonts/families/BuilderSans.json"); lvLab.TextSize=12; lvLab.Parent=right; local cntLab=lvLab:Clone(); cntLab.Position=UDim2.fromOffset(14,198); cntLab.Text="จำนวนตัว Lv เป้าหมายทั้งหมด"; cntLab.Parent=right
 local prog=Instance.new("TextLabel"); prog.BackgroundTransparency=1; prog.Position=UDim2.fromOffset(14,272); prog.Size=UDim2.fromOffset(348,48); prog.TextWrapped=true; prog.TextXAlignment=Enum.TextXAlignment.Left; prog.TextYAlignment=Enum.TextYAlignment.Top; prog.FontFace=Font.new("rbxasset://fonts/families/BuilderSans.json",Enum.FontWeight.Medium,Enum.FontStyle.Normal); prog.TextSize=13; prog.TextColor3=Color3.fromRGB(115,215,180); prog.Text="เลือกตัวก่อน"; prog.Parent=right; local hint=prog:Clone(); hint.Position=UDim2.fromOffset(14,326); hint.Size=UDim2.fromOffset(348,58); hint.TextColor3=Color3.fromRGB(129,147,178); hint.TextSize=11; hint.Text="\u{E40}\u{E25}\u{E37}\u{E2D}\u{E01}\u{E15}\u{E49}\u{E19}\u{E41}\u{E1A}\u{E1A}\u{E04}\u{E23}\u{E31}\u{E49}\u{E07}\u{E40}\u{E14}\u{E35}\u{E22}\u{E27} \u{2022} \u{E23}\u{E30}\u{E1A}\u{E1A}\u{E04}\u{E38}\u{E21} Clone \u{E43}\u{E2B}\u{E49}\u{E40}\u{E2D}\u{E07} \u{2022} \u{E44}\u{E15}\u{E48}\u{E16}\u{E36}\u{E07} Lv \u{E40}\u{E1B}\u{E49}\u{E32}\u{E41}\u{E25}\u{E49}\u{E27}\u{E42}\u{E04}\u{E25}\u{E19}\u{E0B}\u{E49}\u{E33}\u{E08}\u{E19}\u{E16}\u{E36}\u{E07}\u{E08}\u{E33}\u{E19}\u{E27}\u{E19} \u{2022} \u{E15}\u{E49}\u{E2D}\u{E07}\u{E21}\u{E35}\u{E0A}\u{E48}\u{E2D}\u{E07}\u{E27}\u{E48}\u{E32}\u{E07} 1 \u{E0A}\u{E48}\u{E2D}\u{E07}\u{E15}\u{E2D}\u{E19} Merge \u{2022} \u{E44}\u{E21}\u{E48}\u{E01}\u{E34}\u{E19}\u{E15}\u{E49}\u{E19}\u{E09}\u{E1A}\u{E31}\u{E1A}"; hint.Parent=right
 local function refreshSources() for _,x in ipairs(scroll:GetChildren()) do if x~=lay then x:Destroy() end end; local q=string.lower(search.Text or ""); for _,u in ipairs(inv()) do local ok=eligible(u); local m=meta(u.Name); local hay=string.lower(tostring(u.Name).." "..tostring(u.Mutation or "None").." "..tostring(u.Trait or "")); if ok and (q=="" or string.find(hay,q,1,true)) then local b=Instance.new("TextButton"); b.Size=UDim2.new(1,-6,0,54); b.BackgroundColor3=uid(u)==tostring(L.sourceUUID or "") and Color3.fromRGB(38,58,84) or Color3.fromRGB(23,29,42); b.BorderSizePixel=0; b.TextXAlignment=Enum.TextXAlignment.Left; b.FontFace=Font.new("rbxasset://fonts/families/BuilderSans.json",Enum.FontWeight.Medium,Enum.FontStyle.Normal); b.TextSize=13; b.TextColor3=Color3.fromRGB(236,242,252); b.Text="  "..tostring(u.Name).."\n  "..tostring(m and m.Rarity or "?").." • Lv."..tostring(u.Level or 1).." • "..tostring(u.Mutation or "None")..(tostring(u.Trait or "")~="" and (" • "..tostring(u.Trait)) or ""); b.Parent=scroll; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); b.Activated:Connect(function() L.sourceUUID=uid(u); L.name=tostring(u.Name); L.mutation=tostring(u.Mutation or ""); L.trait=tostring(u.Trait or ""); L.baseLevel=tonumber(u.Level) or 1; L.targetLevel=math.min(maxLv(u),math.max(L.baseLevel,tonumber(L.targetLevel) or L.baseLevel)); if L.targetLevel==L.baseLevel and L.baseLevel<maxLv(u) then L.targetLevel=L.baseLevel+1 end; RT.saveLevelConfig(); refreshSources(); if RT.refreshLevelUI then RT.refreshLevelUI() end end) end end end
 RT.refreshLevelUI=function() local src=highestSource(); if L.name and L.name~="" then local m=meta(L.name); sel.Text="  "..tostring(L.name).." • "..tostring(m and m.Rarity or "?").."\n  เริ่ม Lv."..tostring(L.baseLevel).." • "..tostring(L.mutation=="" and "Normal" or L.mutation)..(L.trait~="" and (" • Trait "..L.trait) or "") else sel.Text="  ยังไม่ได้เลือกตัว\n  เลือกจากรายการด้านซ้าย" end; local ml=src and maxLv(src) or 7; L.targetLevel=math.clamp(math.floor(tonumber(L.targetLevel) or 1),math.max(1,L.baseLevel or 1),ml); L.targetCount=math.clamp(math.floor(tonumber(L.targetCount) or 1),1,99); lvText.Text="Lv."..L.targetLevel; cntText.Text="จำนวน "..L.targetCount; local n=countFinal(); local hs=highestSource(); prog.Text=L.name and ("มี Lv."..L.targetLevel.." แล้ว "..n.."/"..L.targetCount.." • Seed สูงสุด Lv."..tostring(hs and hs.Level or "-").."\n"..tostring(L.status or "")) or "เลือกตัวก่อน"; master.Text=L.enabled and "■ หยุด AUTO LEVEL" or "▶ เริ่ม AUTO LEVEL"; master.BackgroundColor3=L.enabled and Color3.fromRGB(41,140,96) or Color3.fromRGB(47,79,142); status.Text="  "..tostring(L.status or "หยุด") end
 keep(search:GetPropertyChangedSignal("Text"):Connect(refreshSources)); keep(lvMinus.Activated:Connect(function() L.targetLevel=math.max(L.baseLevel or 1,(L.targetLevel or 1)-1); RT.saveLevelConfig(); RT.refreshLevelUI() end)); keep(lvPlus.Activated:Connect(function() local s=highestSource(); L.targetLevel=math.min(s and maxLv(s) or 7,(L.targetLevel or 1)+1); RT.saveLevelConfig(); RT.refreshLevelUI() end)); keep(cntMinus.Activated:Connect(function() L.targetCount=math.max(1,(L.targetCount or 1)-1); RT.saveLevelConfig(); RT.refreshLevelUI() end)); keep(cntPlus.Activated:Connect(function() L.targetCount=math.min(99,(L.targetCount or 1)+1); RT.saveLevelConfig(); RT.refreshLevelUI() end))
 keep(master.Activated:Connect(function() if L.enabled then L.enabled=false; L.status=L.busy and "กำลังหยุดหลังงานปัจจุบัน" or "หยุด"; if not L.busy then restoreClone() end else local src=highestSource(); if not src then L.status="เลือกยูนิตก่อน" else local ok,why=eligible(src); if not ok then L.status=why elseif L.targetLevel>(maxLv(src)) then L.status="Lv เป้าหมายสูงเกิน Rarity" else L.enabled=true; L.status="เริ่ม Auto Level"; RT.saveLevelConfig() end end end; RT.refreshLevelUI() end)); refreshSources(); RT.refreshLevelUI()
end
RT.setupAutoLevel()
RT.dropDataJSON=[==[[{"n":"Byakugou Seal","r":"Secret","t":"Material","a":"75302626181039","s":[{"src":"Main World","cat":"MAIN","w":"ศัตรูปกติ","ch":"0.00245398773%"},{"src":"Main World","cat":"MAIN","w":"Boss","ch":"0.00245398773%"}]},{"n":"Common Essence","r":"Common","t":"Material","a":"108470439263673","s":[{"src":"Main World","cat":"MAIN","w":"ศัตรูปกติ","ch":"0.09969095803%"},{"src":"Main World","cat":"MAIN","w":"Boss","ch":"15.950553284817%"}]},{"n":"Epic Essence","r":"Epic","t":"Material","a":"131575201366539","s":[{"src":"Main World","cat":"MAIN","w":"ศัตรูปกติ","ch":"0.062306848769%"},{"src":"Main World","cat":"MAIN","w":"Boss","ch":"9.969095803011%"}]},{"n":"God Essence","r":"God","t":"Material","a":"132109164209633","s":[{"src":"Main World","cat":"MAIN","w":"ศัตรูปกติ","ch":"0.000623068488%"},{"src":"Main World","cat":"MAIN","w":"Boss","ch":"0.09969095803%"},{"src":"Raid","cat":"RAID","w":"Ant Kingdom","ch":"30%","am":"1"},{"src":"Raid","cat":"RAID","w":"Red Knight's Dungeon","ch":"30%","am":"1"},{"src":"Jinwoo Quest","cat":"EVENT","w":"Admin Event quest completion","ch":"60%","am":"5"}]},{"n":"Hunter's License","r":"God","t":"Material","a":"123937348577622","s":[{"src":"Main World","cat":"MAIN","w":"ศัตรูปกติ","ch":"0.000245398773%"},{"src":"Main World","cat":"MAIN","w":"Boss","ch":"0.000245398773%"}]},{"n":"Ice Dagger","r":"Secret","t":"Material","a":"95231854499688","s":[{"src":"Main World","cat":"MAIN","w":"ศัตรูปกติ","ch":"0.00245398773%"},{"src":"Main World","cat":"MAIN","w":"Boss","ch":"0.00245398773%"}]},{"n":"Legendary Essence","r":"Legendary","t":"Material","a":"107259672936966","s":[{"src":"Main World","cat":"MAIN","w":"ศัตรูปกติ","ch":"0.007476821852%"},{"src":"Main World","cat":"MAIN","w":"Boss","ch":"1.196291496361%"}]},{"n":"Mythic Essence","r":"Mythic","t":"Material","a":"140492793685472","s":[{"src":"Main World","cat":"MAIN","w":"ศัตรูปกติ","ch":"0.003738410926%"},{"src":"Main World","cat":"MAIN","w":"Boss","ch":"0.598145748181%"},{"src":"Spin Wheel","cat":"EVENT","w":"Spin Wheel","ch":"raw Chance=12","am":"5"}]},{"n":"Nuclear Core","r":"Supreme","t":"Material","a":"82263086492516","s":[{"src":"Main World","cat":"MAIN","w":"ศัตรูปกติ","ch":"0.000245398773%"},{"src":"Main World","cat":"MAIN","w":"Boss","ch":"0.000245398773%"}]},{"n":"Rare Essence","r":"Rare","t":"Material","a":"98607738862584","s":[{"src":"Main World","cat":"MAIN","w":"ศัตรูปกติ","ch":"0.074768218523%"},{"src":"Main World","cat":"MAIN","w":"Boss","ch":"11.962914963613%"}]},{"n":"S-Rank Badge","r":"God","t":"Material","a":"79397712359600","s":[{"src":"Main World","cat":"MAIN","w":"ศัตรูปกติ","ch":"0.000613496933%"},{"src":"Main World","cat":"MAIN","w":"Boss","ch":"0.000613496933%"}]},{"n":"Secret Essence","r":"Secret","t":"Material","a":"134980491877899","s":[{"src":"Main World","cat":"MAIN","w":"ศัตรูปกติ","ch":"0.001246136975%"},{"src":"Main World","cat":"MAIN","w":"Boss","ch":"0.19938191606%"},{"src":"Raid","cat":"RAID","w":"Ant Kingdom","ch":"40%","am":"2"},{"src":"Raid","cat":"RAID","w":"Red Knight's Dungeon","ch":"40%","am":"2"},{"src":"Jinwoo Quest","cat":"EVENT","w":"Admin Event quest completion","ch":"60%","am":"5"},{"src":"Spin Wheel","cat":"EVENT","w":"Spin Wheel","ch":"raw Chance=1.5","am":"5"}]},{"n":"Shadow Essence","r":"Supreme","t":"Material","a":"131535291929765","s":[{"src":"Main World","cat":"MAIN","w":"ศัตรูปกติ","ch":"0.000245398773%"},{"src":"Main World","cat":"MAIN","w":"Boss","ch":"0.000245398773%"}]},{"n":"Six Eyes","r":"God","t":"Material","a":"80611887091915","s":[{"src":"Main World","cat":"MAIN","w":"ศัตรูปกติ","ch":"0.001840490798%"},{"src":"Main World","cat":"MAIN","w":"Boss","ch":"0.001840490798%"}]},{"n":"Sovereign's Crown","r":"Supreme","t":"Material","a":"111724784874092","s":[{"src":"Main World","cat":"MAIN","w":"ศัตรูปกติ","ch":"0.000061349693%"},{"src":"Main World","cat":"MAIN","w":"Boss","ch":"0.000061349693%"}]},{"n":"Supreme Essence","r":"Supreme","t":"Material","a":"108202216238478","s":[{"src":"Main World","cat":"MAIN","w":"ศัตรูปกติ","ch":"0.000149536437%"},{"src":"Main World","cat":"MAIN","w":"Boss","ch":"0.023925829927%"},{"src":"Jinwoo Quest","cat":"EVENT","w":"Admin Event quest completion","ch":"60%","am":"1"}]},{"n":"Tailed Chakra","r":"God","t":"Material","a":"85116839772621","s":[{"src":"Main World","cat":"MAIN","w":"ศัตรูปกติ","ch":"0.001840490798%"},{"src":"Main World","cat":"MAIN","w":"Boss","ch":"0.001840490798%"}]},{"n":"Trait Shard","r":"Mythic","t":"Material","a":"113553245959749","s":[{"src":"Main World","cat":"MAIN","w":"ศัตรูปกติ","ch":"0.3%"},{"src":"Main World","cat":"MAIN","w":"Boss","ch":"25%"},{"src":"Raid","cat":"RAID","w":"Ant Kingdom","ch":"100%","am":"15"},{"src":"Raid","cat":"RAID","w":"Red Knight's Dungeon","ch":"100%","am":"15"},{"src":"Jinwoo Quest","cat":"EVENT","w":"Admin Event quest completion","ch":"100%","am":"25"},{"src":"Spin Wheel","cat":"EVENT","w":"Spin Wheel","ch":"raw Chance=18","am":"5"},{"src":"Spin Wheel","cat":"EVENT","w":"Spin Wheel","ch":"raw Chance=8","am":"10"}]},{"n":"Cursed Finger","r":"God","t":"Material","a":"101979971995532","s":[{"src":"Infinite Tower","cat":"TOWER","w":"Floor 76–100","ch":"0.15%"}]},{"n":"Cursed Womb","r":"God","t":"Material","a":"108449072105846","s":[{"src":"Infinite Tower","cat":"TOWER","w":"Floor 101–125","ch":"0.25%"}]},{"n":"God Capsule","r":"God","t":"Lootbox","a":"76298144596792","s":[{"src":"Infinite Tower","cat":"TOWER","w":"Floor 151+","ch":"0.1%"},{"src":"Jinwoo Quest","cat":"EVENT","w":"Admin Event quest completion","ch":"50%","am":"1"}]},{"n":"God's Eye","r":"God","t":"Material","a":"128145522521222","s":[{"src":"Infinite Tower","cat":"TOWER","w":"Floor 126–150","ch":"0.25%"}]},{"n":"Gold Potion","r":"Mythic","t":"Material","a":"131672212732584","s":[{"src":"Infinite Tower","cat":"TOWER","w":"Floor 1–125","ch":"ไม่เปิดเผย final %"}]},{"n":"Hogyoku's Fragment","r":"God","t":"Material","a":"138471681279526","s":[{"src":"Infinite Tower","cat":"TOWER","w":"Floor 251–275","ch":"0.1%"}]},{"n":"Luck Potion","r":"Mythic","t":"Material","a":"92604845215184","s":[{"src":"Infinite Tower","cat":"TOWER","w":"Floor 1–125","ch":"ไม่เปิดเผย final %"}]},{"n":"Sakuna's Fragment","r":"God","t":"Material","a":"118387283857812","s":[{"src":"Infinite Tower","cat":"TOWER","w":"Floor 76–100","ch":"0.25%"}]},{"n":"Secret Capsule","r":"Secret","t":"Lootbox","a":"140064923252508","s":[{"src":"Infinite Tower","cat":"TOWER","w":"Floor 151+","ch":"0.25%"}]},{"n":"Setzu's Husk","r":"Supreme","t":"Material","a":"117130811389273","s":[{"src":"Infinite Tower","cat":"TOWER","w":"Floor 126–150","ch":"0.1%"}]},{"n":"Super Gold Potion","r":"Mythic","t":"Material","a":"111696840114560","s":[{"src":"Infinite Tower","cat":"TOWER","w":"Floor 1+","ch":"ไม่เปิดเผย final %"},{"src":"Raid","cat":"RAID","w":"Ant Kingdom","ch":"35%","am":"1"},{"src":"Raid","cat":"RAID","w":"Red Knight's Dungeon","ch":"35%","am":"1"}]},{"n":"Super Luck Potion","r":"Mythic","t":"Material","a":"84170759009714","s":[{"src":"Infinite Tower","cat":"TOWER","w":"Floor 1+","ch":"ไม่เปิดเผย final %"},{"src":"Raid","cat":"RAID","w":"Ant Kingdom","ch":"35%","am":"1"},{"src":"Raid","cat":"RAID","w":"Red Knight's Dungeon","ch":"35%","am":"1"}]},{"n":"Super Time Potion","r":"Secret","t":"Material","a":"84990781166273","s":[{"src":"Infinite Tower","cat":"TOWER","w":"Floor 76+","ch":"ไม่เปิดเผย final %"},{"src":"Raid","cat":"RAID","w":"Ant Kingdom","ch":"30%","am":"1"},{"src":"Raid","cat":"RAID","w":"Red Knight's Dungeon","ch":"30%","am":"1"},{"src":"Jinwoo Quest","cat":"EVENT","w":"Admin Event quest completion","ch":"60%","am":"3"}]},{"n":"Time Potion","r":"Secret","t":"Material","a":"82651082229580","s":[{"src":"Infinite Tower","cat":"TOWER","w":"Floor 26–125","ch":"ไม่เปิดเผย final %"}]},{"n":"Truth Orb","r":"God","t":"Material","a":"117777430007688","s":[{"src":"Infinite Tower","cat":"TOWER","w":"Floor 126–150","ch":"0.25%"}]},{"n":"Broken Scythe","r":"Supreme","t":"Material","a":"85277043762815","s":[{"src":"Raid","cat":"RAID","w":"Ant Kingdom","ch":"2.5%","am":"1"}]},{"n":"Igros Cape","r":"Epic","t":"Accessory","a":"102897016883205","s":[{"src":"Raid Accessory","cat":"RAID","w":"Red Knight's Dungeon","ch":"5%"}]},{"n":"Igros Helmet","r":"Legendary","t":"Accessory","a":"126333176911893","s":[{"src":"Raid Accessory","cat":"RAID","w":"Red Knight's Dungeon","ch":"1%"}]},{"n":"Igros Sword","r":"Mythic","t":"Accessory","a":"74547342498993","s":[{"src":"Raid Accessory","cat":"RAID","w":"Red Knight's Dungeon","ch":"0.5%"}]},{"n":"King's Carapace","r":"Mythic","t":"Accessory","a":"116132166683510","s":[{"src":"Raid Accessory","cat":"RAID","w":"Ant Kingdom","ch":"0.25%"}]},{"n":"King's Tail","r":"Mythic","t":"Accessory","a":"132145664688972","s":[{"src":"Raid Accessory","cat":"RAID","w":"Ant Kingdom","ch":"0.5%"}]},{"n":"Puf's Wings","r":"Legendary","t":"Accessory","a":"86480096342551","s":[{"src":"Raid Accessory","cat":"RAID","w":"Ant Kingdom","ch":"1%"}]},{"n":"Trading Ticket","r":"Legendary","t":"Material","a":"140043680170001","s":[{"src":"Raid","cat":"RAID","w":"Ant Kingdom","ch":"20%","am":"1"},{"src":"Raid","cat":"RAID","w":"Red Knight's Dungeon","ch":"20%","am":"1"}]},{"n":"Yupi's Wings","r":"Legendary","t":"Accessory","a":"127735825244653","s":[{"src":"Raid Accessory","cat":"RAID","w":"Ant Kingdom","ch":"5%"}]}]]==]
RT.setupDropGuide=function()
 local ok,data=pcall(function() return HttpService:JSONDecode(RT.dropDataJSON or "[]") end)
 if not ok or type(data)~="table" then data={} end
 RT.dropUI={group={},data=data,filter="ALL",selected=nil,buttons={}}
 local U=RT.dropUI
 local function add(o) U.group[#U.group+1]=o; return o end
 local function label(text,x,y,w,h,size,color,weight)
  local l=Instance.new("TextLabel"); l.BackgroundTransparency=1; l.Position=UDim2.fromOffset(x,y); l.Size=UDim2.fromOffset(w,h); l.FontFace=Font.new("rbxasset://fonts/families/BuilderSans.json",weight or Enum.FontWeight.Regular,Enum.FontStyle.Normal); l.TextSize=size or 16; l.TextColor3=color or Color3.fromRGB(220,230,248); l.TextXAlignment=Enum.TextXAlignment.Left; l.TextYAlignment=Enum.TextYAlignment.Center; l.Text=text; l.Parent=frame; return l
 end
 local title=add(label("▣  DROP GUIDE",210,84,300,32,23,Color3.fromRGB(244,248,255),Enum.FontWeight.Bold))
 local sub=add(label("ข้อมูลดรอปจากเกมจริง • 42 ไอเท็ม • 83 แหล่ง",210,112,420,22,13,Color3.fromRGB(145,165,198)))
 local verified=add(label("✓ VERIFIED CLIENT DATA",744,90,206,28,13,Color3.fromRGB(103,232,174),Enum.FontWeight.Bold)); verified.TextXAlignment=Enum.TextXAlignment.Right
 local searchBox=Instance.new("TextBox"); searchBox.Size=UDim2.fromOffset(300,38); searchBox.Position=UDim2.fromOffset(210,140); searchBox.BackgroundColor3=Color3.fromRGB(22,28,42); searchBox.BorderSizePixel=0; searchBox.TextColor3=Color3.fromRGB(245,248,255); searchBox.PlaceholderText="ค้นหาไอเท็ม / แหล่งดรอป..."; searchBox.Text=""; searchBox.ClearTextOnFocus=false; searchBox.FontFace=Font.new("rbxasset://fonts/families/BuilderSans.json",Enum.FontWeight.Regular,Enum.FontStyle.Normal); searchBox.TextSize=16; searchBox.Parent=frame; Instance.new("UICorner",searchBox).CornerRadius=UDim.new(0,9); styleTextBox(searchBox); add(searchBox)
 local filters={{"ALL","ทั้งหมด"},{"MAIN","โลกหลัก"},{"TOWER","Tower"},{"RAID","Raid"},{"EVENT","Event"}}
 local filterButtons={}
 for i,f in ipairs(filters) do local b=mkButton(frame,f[2],520+(i-1)*86,140,80,38); b.TextSize=13; filterButtons[f[1]]=b; add(b) end
 local list=Instance.new("ScrollingFrame"); list.Position=UDim2.fromOffset(210,190); list.Size=UDim2.fromOffset(300,400); list.BackgroundColor3=Color3.fromRGB(15,20,31); list.BorderSizePixel=0; list.ScrollBarThickness=4; list.Parent=frame; stylePanel(list); styleScroll(list); add(list)
 local listPad=Instance.new("UIPadding"); listPad.PaddingTop=UDim.new(0,8); listPad.PaddingBottom=UDim.new(0,8); listPad.PaddingLeft=UDim.new(0,7); listPad.PaddingRight=UDim.new(0,7); listPad.Parent=list
 local listLayout=Instance.new("UIListLayout"); listLayout.Padding=UDim.new(0,7); listLayout.SortOrder=Enum.SortOrder.LayoutOrder; listLayout.Parent=list
 local detail=Instance.new("Frame"); detail.Position=UDim2.fromOffset(522,190); detail.Size=UDim2.fromOffset(428,400); detail.BackgroundColor3=Color3.fromRGB(15,20,31); detail.BorderSizePixel=0; detail.Parent=frame; stylePanel(detail); add(detail)
 local hero=Instance.new("Frame"); hero.Position=UDim2.fromOffset(10,10); hero.Size=UDim2.fromOffset(408,112); hero.BackgroundColor3=Color3.fromRGB(22,29,45); hero.BorderSizePixel=0; hero.Parent=detail; stylePanel(hero)
 local heroIcon=Instance.new("ImageLabel"); heroIcon.BackgroundColor3=Color3.fromRGB(10,14,23); heroIcon.BorderSizePixel=0; heroIcon.Position=UDim2.fromOffset(12,12); heroIcon.Size=UDim2.fromOffset(88,88); heroIcon.ScaleType=Enum.ScaleType.Fit; heroIcon.Parent=hero; Instance.new("UICorner",heroIcon).CornerRadius=UDim.new(0,12)
 local heroName=Instance.new("TextLabel"); heroName.BackgroundTransparency=1; heroName.Position=UDim2.fromOffset(114,14); heroName.Size=UDim2.fromOffset(280,30); heroName.FontFace=Font.new("rbxasset://fonts/families/BuilderSans.json",Enum.FontWeight.Bold,Enum.FontStyle.Normal); heroName.TextSize=22; heroName.TextColor3=Color3.fromRGB(248,250,255); heroName.TextXAlignment=Enum.TextXAlignment.Left; heroName.TextTruncate=Enum.TextTruncate.AtEnd; heroName.Parent=hero
 local heroRarity=Instance.new("TextLabel"); heroRarity.BackgroundColor3=Color3.fromRGB(50,60,82); heroRarity.BorderSizePixel=0; heroRarity.Position=UDim2.fromOffset(114,50); heroRarity.Size=UDim2.fromOffset(112,27); heroRarity.FontFace=Font.new("rbxasset://fonts/families/BuilderSans.json",Enum.FontWeight.Bold,Enum.FontStyle.Normal); heroRarity.TextSize=13; heroRarity.TextColor3=Color3.new(1,1,1); heroRarity.Parent=hero; Instance.new("UICorner",heroRarity).CornerRadius=UDim.new(1,0)
 local heroType=Instance.new("TextLabel"); heroType.BackgroundTransparency=1; heroType.Position=UDim2.fromOffset(238,50); heroType.Size=UDim2.fromOffset(150,27); heroType.FontFace=Font.new("rbxasset://fonts/families/BuilderSans.json",Enum.FontWeight.Regular,Enum.FontStyle.Normal); heroType.TextSize=14; heroType.TextColor3=Color3.fromRGB(154,172,205); heroType.TextXAlignment=Enum.TextXAlignment.Left; heroType.Parent=hero
 local heroCount=Instance.new("TextLabel"); heroCount.BackgroundTransparency=1; heroCount.Position=UDim2.fromOffset(114,80); heroCount.Size=UDim2.fromOffset(278,22); heroCount.FontFace=Font.new("rbxasset://fonts/families/BuilderSans.json",Enum.FontWeight.Regular,Enum.FontStyle.Normal); heroCount.TextSize=13; heroCount.TextColor3=Color3.fromRGB(116,205,255); heroCount.TextXAlignment=Enum.TextXAlignment.Left; heroCount.Parent=hero
 local sourceScroll=Instance.new("ScrollingFrame"); sourceScroll.Position=UDim2.fromOffset(10,132); sourceScroll.Size=UDim2.fromOffset(408,258); sourceScroll.BackgroundTransparency=1; sourceScroll.BorderSizePixel=0; sourceScroll.ScrollBarThickness=4; sourceScroll.Parent=detail; styleScroll(sourceScroll)
 local sourceLayout=Instance.new("UIListLayout"); sourceLayout.Padding=UDim.new(0,8); sourceLayout.SortOrder=Enum.SortOrder.LayoutOrder; sourceLayout.Parent=sourceScroll
 local function sourceColor(cat) if cat=="MAIN" then return Color3.fromRGB(53,154,235) elseif cat=="TOWER" then return Color3.fromRGB(146,90,229) elseif cat=="RAID" then return Color3.fromRGB(232,139,55) elseif cat=="EVENT" then return Color3.fromRGB(63,196,139) end return Color3.fromRGB(105,120,150) end
 local function pct(v) v=math.min(100,tonumber(v) or 0); local s=string.format("%.3f",v):gsub("0+$",""):gsub("%.$",""); return s.."%" end
 local function raidDifficulties(ch)
  local n=tonumber(tostring(ch):match("([%d%.]+)%%")); if not n then return nil end
  return "Easy "..pct(n).."  •  Medium "..pct(n*1.25).."  •  Hard "..pct(n*1.5).."  •  Extreme "..pct(n*2)
 end
 local function noteFor(s)
  if s.src=="Main World" then return "อัตราจาก ItemsInfo / GetItemChance ของเกม" end
  if s.src=="Infinite Tower" then return "ยืนยัน Floor และ Chance จาก InfiniteInfo / GetMobItems" end
  if s.src=="Raid" or s.src=="Raid Accessory" then return raidDifficulties(s.ch) or "Raid ใช้ตัวคูณตามระดับความยาก" end
  if s.src=="Jinwoo Quest" then return "Chance ตรงจาก JinwooQuest ของ Admin Event" end
  if s.src=="Spin Wheel" then return "ค่า raw Chance จาก Client • final % ถูกเลือกฝั่ง Server จึงไม่เดา" end
  return "ข้อมูลจาก Client ที่ตรวจยืนยันได้"
 end
 local function clearSourceCards() for _,c in ipairs(sourceScroll:GetChildren()) do if not c:IsA("UIListLayout") then c:Destroy() end end end
 local function renderDetail(it)
  if not it then heroName.Text="เลือกไอเท็มด้านซ้าย"; heroIcon.Image=""; heroRarity.Text=""; heroType.Text=""; heroCount.Text=""; clearSourceCards(); return end
  U.selected=it.n; heroName.Text=tostring(it.n); heroIcon.Image=(it.a and it.a~="") and ("rbxassetid://"..tostring(it.a)) or ""; heroRarity.Text=tostring(it.r or "Unknown"); heroRarity.BackgroundColor3=shadeColor(tierColor(it.r),0.72); heroType.Text=tostring(it.t or "Item"); heroCount.Text=tostring(#(it.s or {})).." แหล่งดรอป / โอกาส"
  clearSourceCards()
  for _,s in ipairs(it.s or {}) do
   local card=Instance.new("Frame"); card.Size=UDim2.new(1,-5,0,94); card.BackgroundColor3=Color3.fromRGB(22,29,43); card.BorderSizePixel=0; card.Parent=sourceScroll; Instance.new("UICorner",card).CornerRadius=UDim.new(0,10)
   local st=Instance.new("UIStroke"); st.Color=sourceColor(s.cat); st.Transparency=0.33; st.Thickness=1.1; st.Parent=card
   local src=Instance.new("TextLabel"); src.BackgroundTransparency=1; src.Position=UDim2.fromOffset(12,7); src.Size=UDim2.fromOffset(245,23); src.FontFace=Font.new("rbxasset://fonts/families/BuilderSans.json",Enum.FontWeight.Bold,Enum.FontStyle.Normal); src.TextSize=15; src.TextColor3=sourceColor(s.cat); src.TextXAlignment=Enum.TextXAlignment.Left; src.Text=tostring(s.src).."  •  "..tostring(s.w); src.TextTruncate=Enum.TextTruncate.AtEnd; src.Parent=card
   local chance=Instance.new("TextLabel"); chance.BackgroundColor3=shadeColor(sourceColor(s.cat),0.34); chance.BorderSizePixel=0; chance.Position=UDim2.fromOffset(268,7); chance.Size=UDim2.fromOffset(124,25); chance.FontFace=Font.new("rbxasset://fonts/families/BuilderSans.json",Enum.FontWeight.Bold,Enum.FontStyle.Normal); chance.TextSize=14; chance.TextColor3=Color3.new(1,1,1); chance.Text=tostring(s.ch); chance.Parent=card; Instance.new("UICorner",chance).CornerRadius=UDim.new(1,0)
   local amount=Instance.new("TextLabel"); amount.BackgroundTransparency=1; amount.Position=UDim2.fromOffset(12,34); amount.Size=UDim2.fromOffset(380,20); amount.FontFace=Font.new("rbxasset://fonts/families/BuilderSans.json",Enum.FontWeight.Medium,Enum.FontStyle.Normal); amount.TextSize=13; amount.TextColor3=Color3.fromRGB(210,220,238); amount.TextXAlignment=Enum.TextXAlignment.Left; amount.Text=(s.am and tostring(s.am)~="") and ("จำนวน: x"..tostring(s.am)) or "จำนวน: ตามระบบดรอป"; amount.Parent=card
   local note=Instance.new("TextLabel"); note.BackgroundTransparency=1; note.Position=UDim2.fromOffset(12,55); note.Size=UDim2.fromOffset(380,31); note.FontFace=Font.new("rbxasset://fonts/families/BuilderSans.json",Enum.FontWeight.Regular,Enum.FontStyle.Normal); note.TextSize=12; note.TextColor3=Color3.fromRGB(148,163,190); note.TextWrapped=true; note.TextXAlignment=Enum.TextXAlignment.Left; note.TextYAlignment=Enum.TextYAlignment.Top; note.Text=noteFor(s); note.Parent=card
  end
 end
 local function hasCat(it,cat) if cat=="ALL" then return true end for _,s in ipairs(it.s or {}) do if s.cat==cat then return true end end return false end
 local function textMatch(it,q)
  if q=="" then return true end; q=string.lower(q); if string.find(string.lower(tostring(it.n)),q,1,true) then return true end
  for _,s in ipairs(it.s or {}) do if string.find(string.lower(tostring(s.src).." "..tostring(s.w)),q,1,true) then return true end end; return false
 end
 local function clearListCards() for _,c in ipairs(list:GetChildren()) do if not c:IsA("UIListLayout") and not c:IsA("UIPadding") then c:Destroy() end end end
 local function rebuild()
  clearListCards(); local q=tostring(searchBox.Text or ""); local shown=0; local first=nil
  for i,it in ipairs(U.data) do if hasCat(it,U.filter) and textMatch(it,q) then
   shown+=1; if not first then first=it end
   local card=Instance.new("TextButton"); card.Name="DropItem_"..tostring(i); card.Size=UDim2.new(1,-3,0,68); card.BackgroundColor3=Color3.fromRGB(22,29,43); card.BorderSizePixel=0; card.Text=""; card.AutoButtonColor=false; card.Parent=list; Instance.new("UICorner",card).CornerRadius=UDim.new(0,10)
   local st=Instance.new("UIStroke"); st.Color=tierColor(it.r); st.Transparency=(U.selected==it.n) and 0.05 or 0.58; st.Thickness=(U.selected==it.n) and 1.8 or 1; st.Parent=card
   local icon=Instance.new("ImageLabel"); icon.BackgroundColor3=Color3.fromRGB(10,14,23); icon.BorderSizePixel=0; icon.Position=UDim2.fromOffset(7,7); icon.Size=UDim2.fromOffset(54,54); icon.Image=(it.a and it.a~="") and ("rbxassetid://"..tostring(it.a)) or ""; icon.ScaleType=Enum.ScaleType.Fit; icon.Parent=card; Instance.new("UICorner",icon).CornerRadius=UDim.new(0,9)
   local nm=Instance.new("TextLabel"); nm.BackgroundTransparency=1; nm.Position=UDim2.fromOffset(70,7); nm.Size=UDim2.new(1,-78,0,24); nm.FontFace=Font.new("rbxasset://fonts/families/BuilderSans.json",Enum.FontWeight.Bold,Enum.FontStyle.Normal); nm.TextSize=15; nm.TextColor3=Color3.fromRGB(242,246,255); nm.TextXAlignment=Enum.TextXAlignment.Left; nm.TextTruncate=Enum.TextTruncate.AtEnd; nm.Text=tostring(it.n); nm.Parent=card
   local rr=Instance.new("TextLabel"); rr.BackgroundTransparency=1; rr.Position=UDim2.fromOffset(70,31); rr.Size=UDim2.new(1,-78,0,18); rr.FontFace=Font.new("rbxasset://fonts/families/BuilderSans.json",Enum.FontWeight.Medium,Enum.FontStyle.Normal); rr.TextSize=12; rr.TextColor3=tierColor(it.r); rr.TextXAlignment=Enum.TextXAlignment.Left; rr.Text=tostring(it.r or "Unknown").."  •  "..tostring(#(it.s or {})).." แหล่ง"; rr.Parent=card
   local cats={}; for _,s in ipairs(it.s or {}) do cats[s.cat]=true end; local cs={}; for _,k in ipairs({"MAIN","TOWER","RAID","EVENT"}) do if cats[k] then cs[#cs+1]=k end end
   local srcs=Instance.new("TextLabel"); srcs.BackgroundTransparency=1; srcs.Position=UDim2.fromOffset(70,49); srcs.Size=UDim2.new(1,-78,0,15); srcs.FontFace=Font.new("rbxasset://fonts/families/BuilderSans.json",Enum.FontWeight.Regular,Enum.FontStyle.Normal); srcs.TextSize=10; srcs.TextColor3=Color3.fromRGB(135,151,180); srcs.TextXAlignment=Enum.TextXAlignment.Left; srcs.Text=table.concat(cs," • "); srcs.Parent=card
   keep(card.MouseButton1Click:Connect(function() U.selected=it.n; renderDetail(it); rebuild() end))
  end end
  sub.Text="ข้อมูลดรอปจากเกมจริง • แสดง "..shown.." / "..#U.data.." ไอเท็ม • 83 แหล่ง"
  local target=nil; if U.selected then for _,it in ipairs(U.data) do if it.n==U.selected and hasCat(it,U.filter) and textMatch(it,q) then target=it; break end end end; if not target then target=first end; renderDetail(target)
 end
 for key,b in pairs(filterButtons) do keep(b.MouseButton1Click:Connect(function() U.filter=key; for k,bb in pairs(filterButtons) do bb.BackgroundColor3=(k==key) and Color3.fromRGB(53,78,176) or Color3.fromRGB(27,34,49) end; rebuild() end)) end
 keep(searchBox:GetPropertyChangedSignal("Text"):Connect(function() rebuild() end))
 RT.refreshDropUI=function() if not RT.alive then return end; for k,b in pairs(filterButtons) do b.BackgroundColor3=(k==U.filter) and Color3.fromRGB(53,78,176) or Color3.fromRGB(27,34,49) end; rebuild() end
 RT.refreshDropUI()
end
...
local tabGroups={ROLL=rollGroup,RULES=rulesGroup,TRADER=traderGroup,INFO=infoGroup,EVENT=eventGroup,CLONE=RT.cloneUI.group or {},LEVEL=RT.levelUI.group or {},DROP=RT.dropUI.group or {},CONFIG=configGroup}
refreshTabs=function()
 ...
 if active=="LEVEL" and not minimized and RT.refreshLevelUI then RT.refreshLevelUI() end
 if active=="DROP" and not minimized and RT.refreshDropUI then RT.refreshDropUI() end
...
local function applyConfigToUI(message)
 ...
 refreshCharButtons(); refreshMutButtons(); refreshRarityButtons(); refreshRulesUI(); refreshTraderButtons(); refreshInfoList(); if infoSelected then selectInfoCharacter(infoSelected) end; if RT.refreshEventUI then RT.refreshEventUI() end; if RT.refreshCloneUI then RT.refreshCloneUI() end; if RT.refreshLevelUI then RT.refreshLevelUI() end
...
local function requestTraderStock(source)
 if not TraderGetStock then return end
...
tryTraderBuy=function()
 if not TraderBuy then return end
...
if TraderBuy then keep(TraderBuy.OnClientEvent:Connect(function(stock)
...
RT.restoreTeleportResume=function()
 ...
 local ok,text=pcall(readfile,"KGHUB_v702_resume.json")
 if not ok or type(text)~="string" or text=="" then return false,"no_resume" end
 if type(delfile)=="function" then pcall(delfile,"KGHUB_v702_resume.json") end
 local ok2,data=pcall(function() return HttpService:JSONDecode(text) end)
 ...
 RT.traderAuto=data.traderAuto==true; RT.traderBusy=false; RT.traderPending=nil; if RT.clone then RT.clone.enabled=data.autoClone==true; RT.clone.busy=false; RT.clone.nextActionAt=os.clock() end; if RT.autoLevel then RT.autoLevel.enabled=data.autoLevel==true; RT.autoLevel.busy=false end
 ...
 if RT.refreshCloneUI then RT.refreshCloneUI() end; if RT.refreshLevelUI then RT.refreshLevelUI() end
...
RT.restoreAutoStateFallback=function()
 ...
 local sameJobBoot=false
 if oldJob~="" and oldJob==newJob and type(readfile)=="function" then local okBoot,boot=pcall(readfile,"KGHUB_TP_BOOT_V1.txt"); if okBoot and type(boot)=="string" then local ts=tonumber(string.match(boot,"^(%d+)")); if ts and math.abs(os.time()-ts)<=45 then sameJobBoot=true end end end
 if oldJob=="" then log("AUTOSTATE_SKIP | reason=empty_job"); return false,"empty_job" end
 if oldJob==newJob and not sameJobBoot then log("AUTOSTATE_SKIP | reason=same_job"); return false,"same_job" end
 if sameJobBoot then log("AUTOSTATE_SAME_JOB_TP_BOOT | age="..tostring(age).." | level="..tostring(data.autoLevel)) end
 RT.autoRoll=data.autoRoll==true; RT.busy=false; RT.nextRollAt=0; RT.rollBuyHold=false; RT.rollBuyBatch=nil
 ...
 if RT.clone then RT.clone.enabled=data.autoClone==true; RT.clone.busy=false; RT.clone.nextActionAt=os.clock() end; if RT.autoLevel then RT.autoLevel.enabled=data.autoLevel==true; RT.autoLevel.busy=false end
...
if (tpResumeOK or rerunResumeOK or autoStateFallbackOK) and type(delfile)=="function" then pcall(delfile,"KGHUB_TP_BOOT_V1.txt") end
...
log("START | version=ui-v7.0.2-production | pool="..#charCatalog.." | mutations="..#mutCatalog.." | rules="..#RT.rules.." | delay="..delayNow().." | config="..CONFIG_FILE.." | rulesFile="..RULES_FILE.." | traderCatalog="..TRADER_CATALOG)
refresh("Drop Guide + Responsive UI | F5 Trader | F6 Roll | F7 Auto Buy")
print("[อาซ้อฟ่างKGHUB ui-v7.0.2-production] loaded | pool="..#charCatalog.." mutations="..#mutCatalog.." log="..LOG)
end
startRuntime()
